        ...
        <div class="module-body column">
          ...
          <form class="form-horizontal row-fluid">
            ...
            <div class="control-group">
              ...
              <div class="controls">
                <input type="number" id="max_allow" data-form-field="max_allowed" placeholder="Enter the max allow value"
                  class="span12">
              </div>
            </div>
          ...
          <table class="table table-striped" style="margin-left: 30px">
            ...
            <tbody>
              @foreach ($studentCategory as $studentCategory)
                <tr>
                  <td>{{ $studentCategory->cat_id }}</td>
                  <td>{{ $studentCategory->category }}</td>
                  <td>{{ $studentCategory->max_allowed }}</td>
                  <td>
                    <div class="btn btn-group">
                      <a href=""><i class="icon-edit"></i></a>
                      <a href="#" data-toggle="modal"
                        data-target="#deleteCategory{{ $studentCategory->cat_id }}"><i class="icon-trash"></i></a>
                    </div>
                  </td>
                </tr>
                <!-- Modal -->
                <div class="modal fade" id="deleteCategory{{ $studentCategory->cat_id }}" tabindex="-1"
                  ...
                  <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-body">
                        <form action="{{ route('student.destroy', $studentCategory->cat_id) }}" method="Post">
                          @csrf
                          @method('Delete')
                          <p>Are you sure you want to delete this ({{ $studentCategory->category }}) Student Category ?
                          </p>
                          <input type="hidden" name="category" id="category"
                            value="{{ $studentCategory->category }}">
                      </div>
                      <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        <button type="submit" class="btn btn-danger">Delete</button>
